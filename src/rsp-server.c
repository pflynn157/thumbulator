...
rsp_continue (struct rsp_buf *buf)
{
  ...
  if (0 == strcmp ("c", buf->data))
    {
      addr = cpu.gpr[15];	/* Default uses current NPC */
    }
...
rsp_read_all_regs ()
{
  ...
  for (r = 0; r < MAX_GPRS; r++)
    {
      reg2hex (cpu.gpr[r], &(buf.data[r * 8]));
    }

  // Get the adjusted PC
  reg2hex (get_pc(), &(buf.data[PC_REGNUM* 8]));

  /* PPC, NPC and SR */
  //reg2hex (cpu.gpr[PPC_REGNUM], &(buf.data[PPC_REGNUM * 8]));
  //reg2hex (cpu.gpr[NPC_REGNUM], &(buf.data[NPC_REGNUM * 8]));
  //reg2hex (cpu.gpr[SR_REGNUM],  &(buf.data[SR_REGNUM  * 8]));
...
rsp_read_mem (struct rsp_buf *buf)
{
  ...
  //fprintf(stderr, "rsp_read_mem: m%x,%x\n", addr,len);
  ...
  for (off = 0; off < len; off++)
      ...
      if (!simValidMem(addr + off))
	{
      ...
      //    continue;
      //  }
      //  buf->data[(off+i-cutBytes)*2] = hexchars[((ch >> 8*i)>>4) & 0xf];
      //  buf->data[(off+i-cutBytes)*2+1] = hexchars[(ch >> 8*i) & 0xf];
      //}
...
rsp_write_mem (struct rsp_buf *buf)
{
  ...
  for (off = 0; off < len; off++)
    ...
    simDebugWrite(addr+off, (nyb1 << 4) | nyb2); 
...
rsp_read_reg (struct rsp_buf *buf)
{
  ...
  if (regnum < MAX_GPRS)
    {
      reg2hex (cpu.gpr[regnum], buf->data);
    }
  else if (regnum == PC_REGNUM)
  {
    reg2hex(get_pc(), buf->data);
  }
  else if (CPSR_REGNUM == regnum)
  {
    reg2hex(cpu.apsr, buf->data);
  }
...
rsp_write_reg (struct rsp_buf *buf)
{
  ...
  if (regnum < MAX_GPRS)
    {
      cpu.gpr[regnum] = hex2reg (valstr);
    }
  else if (regnum == PC_REGNUM)
  {
    set_pc(hex2reg(valstr));
  }
  else if (regnum == CPSR_REGNUM)
  {
    //cpu_reset();
...
rsp_query (struct rsp_buf *buf)
{
  if (0 == strcmp ("qAttached", buf->data))
  ...
  else if (0 == strncmp ("qRcmd,", buf->data, strlen ("qRcmd,")))
    {
      ...
      //rsp_command (buf);
      fprintf (stderr, "Warning: RSP qRcmd query not supported\n");
    }
  ...
  //    put_str_packet ("");
  //}
  //else if (0 == strncmp ("qTfP", buf->data, strlen ("qTfV")))
  //{
  //    put_str_packet ("");
  //}
  else
...
rsp_write_mem_bin (struct rsp_buf *buf)
{
  ...
  for (off = 0; off < len; off++)
    ...
    simDebugWrite(addr+off, bindat[off]);
	}
    }
...
rsp_remove_matchpoint (struct rsp_buf *buf)
{
  ...
  //    if (NULL != mpe)
    ...
    case WP_WRITE:
      mpe = mp_hash_delete (type, addr);
      put_str_packet ("OK");		/* Not supported */
      return;
...
void rsp_check_stall()
{
  ...
  // Watchpoint that fires every x cycles
  else if( NULL != mp_hash_lookup(WP_WRITE,WATCHPOINT_ADDR) && cyclesSinceReset >= resetAfterCycles)
  {
    rsp.stalled = 1;
    rsp.stepping = 0;
    put_str_packet("T05watch:80000010;");
  }
}
...
void rsp_check_watch(unsigned int addr)
{
  ...
  if( NULL != mp_hash_lookup(WP_WRITE, addr))
  {
    rsp.stalled = 1;
    rsp.stepping = 0;
    snprintf(buff, 30, "T05watch:%08X;",addr);
    put_str_packet(buff);
  }
  }
}
